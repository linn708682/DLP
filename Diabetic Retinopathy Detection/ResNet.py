...
def train(model, train_loader, test_loader, optimizer, device, num_class, name_type):
    ...
    for epoch in range(1, epochs+1):
        ...
        acc_test,_ = evaulate(model, test_loader, device, num_class) #test
...
if __name__== "__main__":
    ...
    # test_loader = DataLoader(test_dataset, batch_size=batch_size_18, shuffle=False, num_workers=4)


    # #without_pretrain
    # model=ResNet18(num_class=5,pretrained=False)
    # model = nn.DataParallel(model)
    # model.load_state_dict(torch.load(os.path.join('./models','resnet18_wo_pretrain.pth')),False)
    # model=model.to(device)
    # acc,_ = evaulate(model, test_loader, device, num_class=5)
    # print("resnet18_wo_pretrain.pth:", acc, "%")
    
    # #with_pretrain
    # model=ResNet18(num_class=5,pretrained=True)
    # model = nn.DataParallel(model)
    # model.load_state_dict(torch.load(os.path.join('./models','resnet18_with_pretrain.pth')),False)
    # model=model.to(device)
    # acc,_ = evaulate(model, test_loader, device, num_class=5)
    ...
    # # #without_pretrain
    # model=ResNet50(num_class=5,pretrained=False)
    # model = nn.DataParallel(model)
    # model.load_state_dict(torch.load(os.path.join('./models','resnet50_wo_pretrain.pth')),False)
    # model=model.to(device)
    # acc,_ = evaulate(model, test_loader, device, num_class=5)
    # print("resnet50_wo_pretrain.pth:", acc, "%")


    # #with_pretrain
    # model=ResNet50(num_class=5,pretrained=True)
    # model = nn.DataParallel(model)
    # model.load_state_dict(torch.load(os.path.join('./models','resnet50_with_pretrain.pth')),False)
    # model=model.to(device)
    # acc,_ = evaulate(model, test_loader, device, num_class=5)
